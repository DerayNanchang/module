...
import com.lsn.lib_image_picker.utils.PDateUtil;
        ...
        public void run() {
            ...
            if (!context.isDestroyed() && !cursor.isClosed() && cursor.moveToFirst()) {
                do {
                    ...
                    try {
                        item.id = getLong(cursor, MediaStoreConstants._ID);
                        item.mimeType = getString(cursor, MediaStoreConstants.MIME_TYPE);
                        item.displayName = getString(cursor, MediaStoreConstants.DISPLAY_NAME);
                        ...
                        try {
                            item.path = getString(cursor, MediaStoreConstants.DATA);
                        ...
                        item.width = getInt(cursor, MediaStoreConstants.WIDTH);
                        item.height = getInt(cursor, MediaStoreConstants.HEIGHT);
                        item.setVideo(MimeType.isVideo(item.mimeType));
                        item.time = getLong(cursor, MediaStoreConstants.DATE_MODIFIED);
                    ...
                    //视频
                    if (item.isVideo()) {
                        item.duration = getLong(cursor, MediaStoreConstants.DURATION);
                        if (item.duration == 0) {
                            continue;
                        }
