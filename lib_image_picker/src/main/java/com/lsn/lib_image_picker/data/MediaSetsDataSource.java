    ...
    public void onLoadFinished(@NonNull Loader<Cursor> loader, Cursor cursor) {
        ...
        if (!context.isDestroyed() && cursor.moveToFirst() && !cursor.isClosed()) {
            do {
                ...
                imageSet.id = getString(cursor, MediaStoreConstants.COLUMN_BUCKET_ID);
                imageSet.name = getString(cursor, MediaStoreConstants.COLUMN_BUCKET_DISPLAY_NAME);
                imageSet.coverPath = getString(cursor, MediaStoreConstants.COLUMN_URI);
                imageSet.count = getInt(cursor, MediaStoreConstants.COLUMN_COUNT);
